...
credentials. It defines
  ...
  - the request/result shapes the pipeline speaks (ToolCallRequest,
    ToolCallResult),
  ...
  - LLMClient, the abstract base with four provider primitives
    (call_tool, submit_batch, wait_for_batch, batch_results) and the
    whole-call retry loops built on top of them.

Every pipeline call is a *forced tool call*: one system prompt, one user
prompt, one tool the model must invoke. The retry loops therefore only
...
Retry policy (identical for every caller):
  ...
  - A non-`tool_use` stop reason, a missing tool block, or a response that
    fails the caller's `parse` step is retried; on the final attempt the raw
...
class ToolCallParams(TypedDict):
    """Everything a provider needs to run one forced tool call."""
...
class ToolCallRequest(TypedDict):
    """One tool call, addressable by `custom_id` when batched."""
...
class LLMClient(ABC):
    """Abstract LLM client: four provider primitives plus shared retry loops.

    Subclasses implement the primitives and translate their SDK's errors
    into LLMError / LLMTransientError. Everything else is provider-agnostic
    and inherited.
    """

    def __init__(self, *, max_attempts: int = 2, backoff_seconds: float = 0.0):
        if max_attempts < 1:
    ...
    def call_tool_with_retry(
        ...
        *,
        ...
        component:   label for messages and debug dumps ("curator", …).
        use_batch:   route the call through the batch API (default) or the
                     synchronous API.
